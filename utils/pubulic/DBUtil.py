...
class DBconnect(object):
    def __init__(self,dbinfo_dict,type="oracle",dict_flag=False):
        ...
        if type.lower()=="oracle":
            ...
            self.conn = pymysql.connect(user=username,password=password,host=dbhost,database=dbname,port=int(dbport),charset='utf8')
        else:
                    ...
                    if isinstance(field_list,list) and len(field_list)!=0:
                        ...
                        for item in data_list:
                            ...
                            if len(field_list) == len(item):
                                ...
                                logger.error(f"The length of fields list must be equal to the length of database fields!")
                                break
                    else:
